	...
	class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu
	{
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 )
		{
			...
			if ( !empty( $item->_invalid ) ) {
				...
				$title = sprintf( esc_html__( '%s (Invalid)', 'kuteshop' ), $item->title );
			} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
				$classes[] = 'pending';
				/* translators: %s: title of menu item in draft status */
				$title = sprintf( esc_html__( '%s (Pending)', 'kuteshop' ), $item->title );
			}
			...
			$settings_class = '';
			$settings_btn   = '';
			$settings       = get_post_meta( $item_id, '_ovic_menu_settings', true );
			$settings       = wp_parse_args( $settings, Ovic_Megamenu::$defaults );
			if ( !empty( $settings ) ) {
				if ( $settings['enable_mega'] == 1 ) {
					$settings_class = 'button-primary';
				}
				$menu_icon_type = isset( $settings['menu_icon_type'] ) ? $settings['menu_icon_type'] : 'font-icon';
				if ( $menu_icon_type == 'font-icon' && isset( $settings['menu_icon'] ) && $settings['menu_icon'] != "" ):
					$icon = "<span class=\"{$settings['menu_icon']}\"></span>";
				endif;
				if ( $menu_icon_type == 'image' && isset( $settings['icon_image'] ) && $settings['icon_image'] != "" ):
			...
			$settings_btn .= "<span id=\"ovic-menu-item-settings-{$item_id}\" data-item_id=\"{$item_id}\" ";
			$settings_btn .= "data-item_id=\"{$item_id}\" ";
			$settings_btn .= "data-item_title=\"{$title}\" ";
			$settings_btn .= "class=\"ovic-menu-settings button button-small {$settings_class}\">";
			$settings_btn .= esc_html__( 'SETTINGS', 'kuteshop' );
			$settings_btn .= "</span>";
			?>
                ...
                <span class="item-title">
                    <span class="menu-item-title">
                        ...
                        <span class="menu-label">
                            <?php echo wp_specialchars_decode( $label ); ?>
                        </span>
                    </span>
                    <span class="is-submenu" <?php echo esc_attr( $submenu_text ); ?>><?php _e( 'sub item', 'kuteshop' ); ?></span>
					<?php echo wp_specialchars_decode( $settings_btn ); ?>
                </span>
                    <span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up"
                               aria-label="<?php esc_attr_e( 'Move up', 'kuteshop' ) ?>">&#8593;</a>
							|
							<a href="<?php
							echo wp_nonce_url(
								add_query_arg(
							...
							?>" class="item-move-down"
                               aria-label="<?php esc_attr_e( 'Move down', 'kuteshop' ) ?>">&#8595;</a>
						</span>
						<a class="item-edit" id="edit-<?php echo esc_attr( $item_id ); ?>" href="<?php
						echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
						?>" aria-label="<?php esc_attr_e( 'Edit menu item', 'kuteshop' ); ?>"><span
                                    class="screen-reader-text"><?php _e( 'Edit', 'kuteshop' ); ?></span></a>
					</span>
                </div>
            </div>
            ...
            <div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
                ...
                <p class="description description-wide">
                    <label for="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>">
						<?php _e( 'Navigation Label', 'kuteshop' ); ?><br/>
                ...
                <p class="field-title-attribute field-attr-title description description-wide">
                    <label for="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>">
						<?php _e( 'Title Attribute', 'kuteshop' ); ?><br/>
                ...
                <p class="field-link-target description">
                    <label for="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>">
						...
						<?php _e( 'Open link in a new tab', 'kuteshop' ); ?>
                    </label>
                </p>
                <p class="field-css-classes description description-thin">
                    <label for="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>">
						<?php _e( 'CSS Classes (optional)', 'kuteshop' ); ?><br/>
                ...
                <p class="field-xfn description description-thin">
                    <label for="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>">
						<?php _e( 'Link Relationship (XFN)', 'kuteshop' ); ?><br/>
                ...
                <p class="field-description description description-wide">
                    <label for="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>">
						<?php _e( 'Description', 'kuteshop' ); ?><br/>
                        <textarea id="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>"
							...
							?></textarea>
                        <span class="description"><?php _e( 'The description will be displayed in the menu if the current theme supports it.', 'kuteshop' ); ?></span>
                    </label>
                </p>

                <fieldset class="field-move hide-if-no-js description description-wide">
                    <span class="field-move-visual-label"
                          aria-hidden="true"><?php _e( 'Move', 'kuteshop' ); ?></span>
                    <button type="button" class="button-link menus-move menus-move-up"
                            data-dir="up"><?php _e( 'Up one', 'kuteshop' ); ?></button>
                    <button type="button" class="button-link menus-move menus-move-down"
                            data-dir="down"><?php _e( 'Down one', 'kuteshop' ); ?></button>
                    <button type="button" class="button-link menus-move menus-move-left" data-dir="left"></button>
                    <button type="button" class="button-link menus-move menus-move-right" data-dir="right"></button>
                    <button type="button" class="button-link menus-move menus-move-top"
                            data-dir="top"><?php _e( 'To the top', 'kuteshop' ); ?></button>
                </fieldset>

                <div class="menu-item-actions description-wide submitbox">
					<?php if ( 'custom' != $item->type && $original_title !== false ) : ?>
                        <p class="link-to-original">
							<?php printf( esc_html__( 'Original: %s', 'kuteshop' ), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                        </p>
					<?php endif; ?>
                    <a class="item-delete submitdelete deletion" id="delete-<?php echo esc_attr( $item_id ); ?>"
					   ...
					   echo wp_nonce_url(
						   ...
						   'delete-menu_item_' . $item_id
							...
							?>#menu-item-settings-<?php echo esc_attr( $item_id ); ?>"><?php _e( 'Cancel', 'kuteshop' ); ?></a>
                </div>
