	...
	class Ovic_Megamenu_Settings extends Ovic_Megamenu
	{
		...
		public function _theme_setup()
		{
			register_nav_menus( array(
					'ovic_mobile_menu' => esc_html__( 'Mobile Menu', 'kuteshop' ),
				)
			);
		}
		...
		public static function _is_frontend_editor()
		{
		...
		public static function sample_mobile_menu( $menu_locations, $default = 'primary' )
		{
			if ( !empty( $menu_locations ) ) {
				...
				$mobile_menu .= "<span class='ovic-menu-current-panel-title'>" . esc_html__( 'Main Menu', 'kuteshop' ) . "</span>";
		...
		public static function install_mobile_menu( $menu_locations, $default = 'primary', $class = 'mobile-main-menu', $title = '' )
		{
			$menu_title = !empty( $title ) ? $title : esc_html__( 'Main Menu', 'kuteshop' );

			if ( !empty( $menu_locations ) ) : ?>
				<?php
				$count       = 0;
				$data_menus  = self::_return_menu_items( $menu_locations, $default );
				$array_menus = $data_menus['menus'];
				$array_child = $data_menus['child'];
				?>
                <div class="ovic-menu-clone-wrap <?php echo esc_attr( $class ); ?>">

					<?php do_action( 'ovic_before_html_mobile_menu', $menu_locations, $data_menus ); ?>

                    <div class="ovic-menu-panels-actions-wrap">

                        <span class="ovic-menu-current-panel-title"
                              data-main-title="<?php echo esc_attr( $menu_title ); ?>">
                            <?php echo esc_html( $menu_title ); ?>
                        </span>

                        <a href="#" class="ovic-menu-close-btn ovic-menu-close-panels">x</a>

						<?php do_action( 'ovic_title_html_mobile_menu', $menu_locations, $data_menus ); ?>

                    </div><!-- .ovic-menu-panels-actions-wrap -->

					<?php do_action( 'ovic_before_panels_mobile_menu', $menu_locations, $data_menus ); ?>
					...
					<?php do_action( 'ovic_after_html_mobile_menu', $menu_locations, $data_menus ); ?>
		...
		public function _clone_mobile_menu()
		{
			...
			if ( !empty( $menus ) ) {
				...
				echo apply_filters( 'ovic_menu_toggle_mobile', '<button class="menu-toggle"><span>' . esc_html__( 'Menu Mobile', 'kuteshop' ) . '</span></button>' );
			}
		}
		...
		public static function _return_title_menu_item( $menu_item, $settings )
		{
			...
			if ( $menu_icon_type == 'font-icon' ) {
				...
				if ( isset( $settings['icon_image'] ) && $settings['icon_image'] != '' && $settings['icon_image'] > 0 ) {
					$image = wp_get_attachment_image( $settings['icon_image'], 'full', false, array( 'class' => 'icon-image' ) );
					$title = '<figure class="icon">' . $image . '</figure>' . $title;
				}
			}
			if ( isset( $settings['label_image'] ) && $settings['label_image'] != '' && $settings['label_image'] > 0 ) {
				$image = wp_get_attachment_image( $settings['label_image'], 'full', false, array( 'class' => 'label-image' ) );
				$title = $title . $image;
			}
		...
		public static function _return_megamenu_item( $menu_item, $settings, $depth = 0 )
		{
			...
			if ( $enable_mega == true ) {
				...
				if ( $menu_content_id > 0 && $enable_mega == true ) {
					...
					if ( isset( $settings['menu_bg'] ) && $settings['menu_bg'] > 0 && $image = wp_get_attachment_image_url( $settings['menu_bg'], 'full' ) ) {
					...
					if ( $image != '' ) {
						$attr['data-src'] = $image;
						$attr['class']    = 'sub-menu megamenu lazy';
					}
					$_output = rtrim( "<div" );
		...
		public function ajax_form_settings()
		{
			...
			$item_id         = isset( $_POST['item_id'] ) ? absint( $_POST['item_id'] ) : '';
			$title           = isset( $_POST['item_title'] ) ? wp_unslash( $_POST['item_title'] ) : '';
			$item_depth      = isset( $_POST['depth'] ) ? wp_unslash( $_POST['depth'] ) : '';
			$settings        = get_post_meta( $item_id, self::$meta_key, true );
			...
			$bg_thumbnail        = !empty( $settings['menu_bg'] ) ? wp_get_attachment_image_url( $settings['menu_bg'], 'thumbnail' ) : $placeholder;
			$icon_image_thumb    = !empty( $settings['icon_image'] ) ? wp_get_attachment_image_url( $settings['icon_image'], 'thumbnail' ) : $placeholder;
			$label_image_thumb   = !empty( $settings['label_image'] ) ? wp_get_attachment_image_url( $settings['label_image'], 'thumbnail' ) : $placeholder;
			$data_template       = array(
				'item_id'        => $item_id,
				'title'          => $title,
		...
		public function save_all_settings()
		{
			...
			if ( !$item_id ) {
				$response['errors'][] = esc_html__( 'Menu item not exists.', 'kuteshop' );
		...
		public function create_mega_menu()
		{
			...
			$item_title    = isset( $_POST['item_title'] ) ? wp_unslash( $_POST['item_title'] ) : '';
			$options_id    = isset( $_POST['options_id'] ) ? wp_unslash( $_POST['options_id'] ) : array();
			$settings      = get_post_meta( $item_id, self::$meta_key, true );
			$settings      = wp_parse_args( ( array )$settings, self::$defaults );
			if ( !$item_id ) {
				$response['errors'][] = esc_html__( 'Menu item not exists.', 'kuteshop' );
