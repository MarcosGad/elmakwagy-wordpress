...
if ( !class_exists( 'MCAPI' ) ) {
	class MCAPI
	{
		...
		private function show_error( $message )
		{
			...
			add_settings_error( 'kuteshop-api', 'kuteshop-api-error', $message, 'error' );
		...
		{
			$message .= '<br /><br />' . sprintf( '<a href="%s">' . __( 'Read more about common connectivity issues.', 'kuteshop-toolkit' ) . '</a>', 'https://kb.mc4wp.com/solving-connectivity-issues/#utm_source=wp-plugin&utm_medium=mailchimp-for-wp&utm_campaign=settings-notice' );
		...
		public function call( $method, array $data = array() )
		{
			...
			$request_args   = array(
				...
				'sslverify' => apply_filters( 'kuteshop_use_sslverify', true ),
			);
		...
		private function get_headers()
		{
			...
			$headers = array(
				...
				'User-Agent' => 'kuteshop/' . KUTESHOP_VERSION . '; WordPress/' . $wp_version . '; ' . get_bloginfo( 'url' ),
			);
