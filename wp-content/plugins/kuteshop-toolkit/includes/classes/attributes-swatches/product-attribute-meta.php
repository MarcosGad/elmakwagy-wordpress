	...
	class Ovic_Attribute_Product_Meta
	{
		...
		public function __construct()
		{
			...
			if ( strpos( $this->taxonomy, 'pa_' ) !== false ) {
				$this->attribute = $this->get_product_attribute( $this->taxonomy );
				if ( in_array( $this->attribute['type'], $this->type ) ) {
					...
					add_filter( 'manage_' . $this->taxonomy . '_custom_column', array( $this, 'product_attr_column' ), 10, 3 );
				}
			}
		...
		{
			$ovic_types = array(
				'color'  => esc_html__( 'Color', 'kuteshop-toolkit' ),
				'button' => esc_html__( 'Button', 'kuteshop-toolkit' ),
				'photo'  => esc_html__( 'Photo', 'kuteshop-toolkit' ),
			);
		...
		{
			if ( in_array( $data['attribute_type'], $this->type ) ) {
				global $wpdb;
				$data['attribute_public'] = isset( $data['has_archives'] ) ? (int)$data['has_archives'] : 0;
				$data['attribute_size']   = isset( $_POST['attribute_size'] ) ? wp_unslash( $_POST['attribute_size'] ) : '20x20';
		...
		public function add_attribute_fields()
		{
            ...
            <div class="form-field" style="display: none;">
                <label><?php esc_html_e( 'Size Box', 'kuteshop-toolkit' ); ?></label>
                <input type="text" name="attribute_size" id="attribute_size" value="20x20">
                <p class="description"><?php esc_html_e( 'Determines Size in this attribute.{width}x{height}', 'kuteshop-toolkit' ); ?></p>
            </div>
		...
		public function edit_attribute_fields()
		{
            ...
            <tr class="form-field form-required" <?php if ( $attribute->attribute_type == 'select' ): ?> style="display: none;" <?php endif; ?>>
                <th scope="row" valign="top"><label><?php _e( 'Size Box', 'kuteshop-toolkit' ); ?></label></th>
                <td>
		...
		public function dokan_option_terms( $attribute_taxonomy, $i )
		{
			...
			if ( 'box_style' === $attribute_taxonomy->attribute_type ) : ?>
                <select multiple="multiple" style="width:100%"
                        data-placeholder="<?php esc_attr_e( 'Select terms', 'kuteshop-toolkit' ); ?>"
                ...
                <div class="dokan-pre-defined-attribute-btn-group">
                    <button class="dokan-btn dokan-btn-default plus dokan-select-all-attributes"><?php _e( 'Select all', 'kuteshop-toolkit' ); ?></button>
                    <button class="dokan-btn dokan-btn-default minus dokan-select-no-attributes"><?php _e( 'Select none', 'kuteshop-toolkit' ); ?></button>
                    <!-- <button class="dokan-btn dokan-btn-default fr plus dokan-add-new-attribute"><?php _e( 'Add new', 'kuteshop-toolkit' ); ?></button> -->
                </div>
			<?php
			endif;
		}
		...
		public function product_option_terms( $attribute_taxonomy, $i )
		{
			...
			if ( in_array( $attribute_taxonomy->attribute_type, $this->type ) ) {
                ...
                <button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'kuteshop-toolkit' ); ?></button>
                <button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'kuteshop-toolkit' ); ?></button>
                <button class="button fr plus add_new_attribute"><?php esc_html_e( 'Add new', 'kuteshop-toolkit' ); ?></button>
				<?php
			}
		}
		...
		public function add_attr_field()
		{
			...
			if ( $this->attribute['type'] == 'color' ): ?>
                <input type="hidden" name="<?php echo esc_attr( $id_type ); ?>" value="color">
                <div class="form-field term-display-type-wrap swatch color">
                    <label><?php _e( '', 'kuteshop-toolkit' ); ?></label>
			...
			<?php elseif ( $this->attribute['type'] == 'photo' ): ?>
                <div class="form-field term-display-type-wrap swatch photo">
                    <label><?php _e( 'Thumbnail', 'kuteshop-toolkit' ); ?></label>
                    ...
                    <div style="line-height: 60px;">
                        <input type="hidden" name="<?php echo esc_attr( $id_type ); ?>" value="photo">
                        <input type="hidden"
                               id="product_attribute_thumbnail_id"
                               name="<?php echo esc_attr( $id_photo ); ?>"/>
                        <button type="button"
                                class="upload_image_button button"><?php _e( 'Upload/Add image', 'kuteshop-toolkit' ); ?></button>
                        <button type="button"
                                class="remove_image_button button"><?php _e( 'Remove image', 'kuteshop-toolkit' ); ?></button>
                    </div>
                    <script type="text/javascript">
                        ...
                        jQuery(document).on('click', '.upload_image_button', function (event) {
                            ...
                            file_frame = wp.media.frames.downloadable_file = wp.media({
                                title: '<?php _e( 'Choose an image', 'kuteshop-toolkit' ); ?>',
                                button: {
                                    text: '<?php _e( 'Use image', 'kuteshop-toolkit' ); ?>'
                                },
                                multiple: false
                            });
			...
			<?php else: ?>
                <input type="hidden" name="<?php echo esc_attr( $id_type ); ?>" value="label">
			<?php
			endif;
		}
		...
		public function edit_attr_field( $term, $taxonomy )
		{
			...
			if ( $this->attribute['type'] == 'color' ): ?>
                <tr class="form-field swatch color active">
                    <th scope="row" valign="top"><label><?php _e( '', 'kuteshop-toolkit' ); ?></label></th>
                    <td>
                        <input type="hidden" name="<?php echo esc_attr( $id_type ); ?>" value="color">
			...
			<?php elseif ( $this->attribute['type'] == 'photo' ): ?>
                <tr class="form-field swatch photo active">
                    <th scope="row" valign="top"><label><?php _e( '', 'kuteshop-toolkit' ); ?></label></th>
                    <td>
                        <label><?php _e( 'Thumbnail', 'kuteshop-toolkit' ); ?></label>
                        <div id="product_attribute_thumbnail" style="float: left; margin-right: 10px;">
		...
		public function product_field_save( $term_id, $tt_id, $taxonomy )
		{
			if ( isset( $_POST['product_attribute_meta'] ) ) {
				$metas = wp_unslash( $_POST['product_attribute_meta'] );
		...
		public function product_attr_columns( $columns )
		{
			...
			$new_columns['thumb'] = __( 'Image', 'kuteshop-toolkit' );
		...
		public function wc_variation_attribute_options( $html, $args )
		{
			...
			$show_option_none_text = $args['show_option_none'] ? $args['show_option_none'] : __( 'Choose an option', 'kuteshop-toolkit' ); // We'll do our best to hide the placeholder, but we'll need to show something when resetting options.
			if ( empty( $options ) && !empty( $product ) && !empty( $attribute ) ) {
				$attributes = $product->get_variation_attributes();
				$options    = $attributes[$attribute];
			}
			if ( !empty( $options ) && taxonomy_exists( $attribute ) && !empty( $this->get_product_attribute( $attribute ) ) ) {
				$html               = '';
				$swatch_width       = 20;
				$swatch_height      = 20;
				...
				foreach ( $terms as $term ) {
					if ( in_array( $term->slug, $options ) ) {
						...
						$photo_url  = wp_get_attachment_image_url( $data_photo, array( $swatch_width, $swatch_height ) );
						if ( !$photo_url )
