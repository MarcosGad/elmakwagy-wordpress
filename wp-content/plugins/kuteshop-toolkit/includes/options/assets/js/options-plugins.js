...
(function ( $ ) {
    ...
    function OVICAjaxChosen( element, options ) {
        this.element = $( element );
        this.options = options;
        this.init();
    };

    OVICAjaxChosen.prototype.init = function () {
    ...
    OVICAjaxChosen.prototype.events = function () {
    ...
    OVICAjaxChosen.prototype.search_field_focused = function () {
    ...
    OVICAjaxChosen.prototype.search_welcome_message = function () {
    ...
    OVICAjaxChosen.prototype.update_list = function () {
        ...
        if ( value === this.search_field.data( 'prevVal' ) ) {
            return;
        }
        ...
        this.timer = setTimeout( function () {
            ...
            _this.chosenXhr = window.wp.ajax.post( 'ovic-chosen', _this.options.data ).done( function ( response ) {
    ...
    OVICAjaxChosen.prototype.show_results = function ( items ) {
        ...
        if ( this.is_multiple ) {
            ...
            var $hidden_select = this.element.parent().find( '.ovic-hidden-select' );
            var $hidden_value  = $hidden_select.val() || [];

            this.element.OVICChosenOrder( $hidden_value, true );
    ...
    $.fn.OVICAjaxChosen = function ( chosenOptions ) {
        return this.each( function () {
            new OVICAjaxChosen( this, chosenOptions );
        } );
    };
...
(function () {
    var $, OVICAbstractChosenOrder, _ref,
    ...
    OVICAbstractChosenOrder = (function () {

        function OVICAbstractChosenOrder() {
        }

        OVICAbstractChosenOrder.insertAt = function ( node, index, parentNode ) {
            return parentNode.insertBefore( node, parentNode.children[index].nextSibling );
        };

        OVICAbstractChosenOrder.getFlattenedOptionsAndGroups = function ( select ) {
        ...
        OVICAbstractChosenOrder.isValidMultipleSelectElement = function ( element ) {
            return element !== null && typeof element !== "undefined" && element.nodeName === "SELECT" && element.multiple;
        };

        OVICAbstractChosenOrder.getChosenUIContainer = function ( select ) {
            if ( select.id !== "" ) {
        ...
        OVICAbstractChosenOrder.isChosenified = function ( select ) {
            return this.getChosenUIContainer( select ) != null;
        };

        OVICAbstractChosenOrder.forceSelection = function ( select, selection ) {
        ...
        OVICAbstractChosenOrder.OVICChosenOrder = function ( select, order, force ) {
            var chosen_choices, chosen_options, chosen_ui, i, j, opt, opt_val, option, options, rel, relAttributeName,
                _i, _j, _len, _len1, _results;
        ...
        return OVICAbstractChosenOrder;
    ...
    $.fn.extend( {
        OVICChosenOrder: function ( order, force ) {
            return _OVICChosenOrder.OVICChosenOrder( this, order, force );
        }
    } );

    this._OVICChosenOrder = (function ( _super ) {
        __extends( _OVICChosenOrder, _super );

        function _OVICChosenOrder() {
            _ref = _OVICChosenOrder.__super__.constructor.apply( this, arguments );
            return _ref;
        }

        _OVICChosenOrder.relAttributeName = 'data-option-array-index';

        _OVICChosenOrder.isjQueryObject = function ( obj ) {
            return (typeof jQuery !== "undefined" && jQuery !== null) && obj instanceof jQuery;
        };

        _OVICChosenOrder.getDOMElement = function ( element ) {
        ...
        _OVICChosenOrder.searchChosenUIContainer = function ( element ) {
        ...
        _OVICChosenOrder.triggerEvent = function ( target, event_name ) {
            return $( target ).trigger( event_name );
        };

        return _OVICChosenOrder;

    })( OVICAbstractChosenOrder );
...
(function ( root, factory ) {
    ...
    //
    // Ovic: Added custom patterns for spesific validate
    //
    var patterns = {
    ...
    //
    // Ovic: Renamed function names for avoid conflicts
    //

    if ( typeof $.fn !== "undefined" ) {
        $.fn.serializeObjectOVIC = FormSerializer.serializeObject;
        $.fn.serializeJSONOVIC   = FormSerializer.serializeJSON;
    }
