	...
	class OVIC_Shortcode extends OVIC_Abstract
	{
		...
		public function __construct( $settings = array(), $options = array() )
		{
			...
			$this->settings = wp_parse_args(
				...
				array(
					...
					'button_title' => esc_html__( 'Add shortcode', 'ovic-addon-toolkit' ),
			...
			if ( ! empty( $this->settings['id'] ) ) {
				...
				if ( get_user_option( 'rich_editing' ) == 'true' ) {
					add_action( 'after_wp_tiny_mce', array( &$this, 'tiny_mce_script' ) );
		...
		*/
		...
		public function shortcode_button( $editor_id )
		{
			ob_start();
			?>
            <a href="#" class="button button-primary ovic-shortcode-button"
				<?php if ( ! empty( $editor_id ) ): ?>
