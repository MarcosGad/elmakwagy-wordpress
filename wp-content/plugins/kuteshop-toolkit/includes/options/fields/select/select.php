		...
		public function render()
		{
			...
			if ( isset( $this->field['options'] ) ) {
				if ( !empty( $args['ajax'] ) ) {
					...
					$args['settings']['data']['nonce'] = wp_create_nonce( 'ovic_chosen_ajax_nonce' );
				...
				$chosen_sortable  = ( $args['chosen'] && $args['sortable'] ) ? ' ovic-chosen-sortable' : '';
				$chosen_ajax      = ( $args['chosen'] && $args['ajax'] ) ? ' ovic-chosen-ajax' : '';
				$placeholder_attr = ( $args['chosen'] && $args['placeholder'] ) ? ' data-placeholder="' . $args['placeholder'] . '"' : '';
				$field_class      = ( $args['chosen'] ) ? ' class="ovic-chosen' . $chosen_rtl . $chosen_sortable . $chosen_ajax . '"' : '';
				...
				if ( is_string( $maybe_options ) && !empty( $args['ajax'] ) ) {
				...
				if ( ( is_array( $options ) && !empty( $options ) ) || !empty( $args['chosen'] ) ) {
					if ( !empty( $args['chosen'] ) && !empty( $args['multiple'] ) ) {
						echo '<select name="' . $field_name . '" class="ovic-hidden-select ovic-hidden"' . $multiple_attr . $field_attr . '>';
					...
					echo ( !empty( $this->field['empty_message'] ) ) ? $this->field['empty_message'] : esc_html__( 'No data provided for this option type.', 'ovic-addon-toolkit' );
				}
			}
