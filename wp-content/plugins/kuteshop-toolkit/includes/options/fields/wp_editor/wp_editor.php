 ...
 *
 */
if ( !class_exists( 'OVIC_Field_wp_editor' ) ) {
	class OVIC_Field_wp_editor extends OVIC_Fields
	{
		...
		public function render()
		{
			...
			echo ( ovic_wp_editor_api() ) ? '<div class="ovic-wp-editor" data-editor-settings="' . esc_attr( wp_json_encode( $editor_settings ) ) . '">' : '';

			echo '<textarea name="' . $this->field_name() . '" ' . $this->field_attributes( $attributes ) . $editor_height . '>' . $this->value . '</textarea>';

			echo '<div class="clear"></div>';

			echo ( ovic_wp_editor_api() ) ? '</div>' : '';
		...
		{
			if ( ovic_wp_editor_api() && function_exists( 'wp_enqueue_editor' ) ) {
				wp_enqueue_editor();
		...
		public function setup_wp_editor_media_buttons()
		{
			...
			echo 'var ovic_media_buttons = ' . wp_json_encode( $media_buttons ) . ';';
			echo '</script>';
		}
