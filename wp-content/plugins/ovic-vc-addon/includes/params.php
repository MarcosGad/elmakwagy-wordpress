...
if ( ! class_exists( 'Ovic_VC_Params' ) ) {
	class Ovic_VC_Params
	{
		...
		public static function generate_slide_attr( $data )
		{
			...
			if ( $data ) {
				if ( is_array( $data ) ) {
					return htmlspecialchars( ' data-slick=' . json_encode( $data ) . ' ' );
				}
				parse_str( html_entity_decode( $data ), $atts );
				...
				$data_slick = array_merge( $atts['settings'],
					array( 'responsive' => array_values( $atts['responsive'] ) ) );
		...
		public static function field_group( $data )
		{
			...
			$html .= '<label for="' . $id . '" class="disable-vertical"><input id="' . $id . '" name="responsive[' . $data['id'] . '][settings][vertical]" type="checkbox" class="value_input" ' . checked( $vertical,
					'false' ) . ' value="false"> Disable Vertical</label>';
			if ( isset( $data['desc'] ) ) {
		...
		public static function field_checkbox( $data )
		{
			...
			$html .= '<input id="settings_' . $data['id'] . '" name="settings[' . $data['id'] . ']" ' . checked( $value,
					true ) . ' type="checkbox" class="value_input" value="true">';
